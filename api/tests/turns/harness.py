"""Scenario runner: MockTransport + FluxStub + [TurnSignalAbsorberProcessor] + the real user
aggregator (VAD stub inside) + mock LLM/TTS. One monotonic clock, absolute deadlines, clean
shutdown (spec §4).

Ported from the 8a spike (``api/tests/spike_hybrid_turn/harness.py`` in the spike worktree)
against the production processors: the absorber under test is
``api.services.pipecat.turns.absorber`` and the aggregator comes from the production seam
``build_context_aggregators(..., hybrid=…)``, so ``absorber=False`` is the real stock pipeline
and doubles as the mutation of the whole feature.
"""
...
from api.tests.turns.analyzer_stub import ScriptedAnalyzer
from api.tests.turns.flux_stub import FluxStub
from api.tests.turns.timeline import Timeline
from api.tests.turns.vad_stub import ScriptedVAD

# Frames carrying user text toward the aggregator. ``HeldTranscriptionFrame`` is a
# ``TranscriptionFrame`` subclass, named here so the tuple lists every transcript-bearing frame
# and the Tap's key says which one crossed (a released hold reads as `DOWN:HeldTranscriptionFrame`).
TEXT_FRAMES = (
    TranscriptionFrame,
    HeldTranscriptionFrame,
    InterimTranscriptionFrame,
...
WATCHED = (
    ...
    UserMuteStartedFrame,
    UserMuteStoppedFrame,
    *TEXT_FRAMES,  # register M2: which interims reach the aggregator, and when
        ...
        if isinstance(frame, WATCHED):
            ...
            self._events.append(
                (
                    t,
                    key
                    + (f" '{frame.text}'" if isinstance(frame, TEXT_FRAMES) else ""),
                )
            )
...
def _dropped_by_absorber(stats: Counter) -> int:
    ...
    a second final for the same turn takes one of the exits above too."""
...
async def run_scenario(
    ...
    *,
    ...
    context = LLMContext(messages=[{"role": "system", "content": "scenario"}])
    # The production seam: with ``hybrid=False`` this is the stock ``LLMContextAggregatorPair``,
    # i.e. the pipeline as it ships today.
    ...
    user_agg._handle_transcription = _counting  # test double on a live instance
    ...
    # ``cancel_on_idle_timeout=False``: a scenario is seconds of simulated audio and its idle
    # window (no BotSpeaking/UserSpeaking frames) may outlast nothing here — the default is 300 s
    # (worker.py:92) — but the scenarios must not depend on that margin.
    worker = PipelineWorker(Pipeline(processors), cancel_on_idle_timeout=False)
    ...
    if sc.bot_speaking_at is not None:
        timeline.at(
            sc.bot_speaking_at,
            lambda: worker.queue_frame(TTSSpeakFrame("bot is talking for a while")),
        )
    timeline.at(sc.end_at, worker.stop_when_done)
    ...
    try:
        await asyncio.wait_for(
            asyncio.gather(timeline.run(), running),
    ...
    stats = Counter(absorber_processor.stats) if absorber_processor else Counter()
    return Result(
        messages=messages,
        counts=counts,
        absorber_stats=stats,
        ...
        dropped_by_absorber=_dropped_by_absorber(stats) if absorber_processor else 0,
        local_turns_completed=analyzer.complete_count,
        stub_events=list(stub.emitted),
        vad_windows=list(vad.windows),
    )
